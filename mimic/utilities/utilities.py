...
def plot_CRM(observed_species, observed_resources, timepoints, csv_file=None):
    ...
    # Plot each resource - using distinct colors that continue from where species left off
    for resource_idx in range(observed_resources.shape[1]):
        # Use a different color index for resources (continuing from where species left off)
        color_idx = observed_species.shape[1] + resource_idx
        # Use modulo to cycle through colors if we have more entities than colors
        color_idx = color_idx % len(cols)
        
        label = f'Resource {resource_idx + 1}'
        ax.plot(timepoints, observed_resources[:, resource_idx], linestyle='--', 
                color=cols[color_idx], label=label)
    ...
    if csv_file:
        ...
        for i in range(num_species):
            ...
            if species_col in data.columns:
                ax.scatter(data[time_col], data[species_col],
                          marker='o', color=cols[i % len(cols)], s=10, alpha=0.7,
                          label=f'Observed {species_col}')
        
        # Plot observed resource data with different markers and consistent colors with simulated resources
...
def compare_params(**kwargs):
    ...
    **kwargs : Each argument should be a tuple of (true_value, predicted_value)
    ...
    for param_name, param_values in kwargs.items():
        ...
        ax.set_xlabel('index')
        ax.set_ylabel(f'{param_name}[i]' if true_array.ndim == 1 else f'{param_name}[i,j]')
        ax.legend()
