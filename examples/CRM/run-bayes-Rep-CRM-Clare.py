...
# Output folder 
output_folder = "Clare_EcN_Sent_EAB12glu_test_norm10"  
...
# # Combine them
# init_species = np.concatenate([y0_species, y0_resources]) 
...
prior_r_mean = 8.0              
prior_r_sigma = 1.0
prior_K_mean = [0.2, 0.5]      
prior_K_sigma = [0.1, 0.1]
...
Notes:
y0_species inferred with mu = 0.05, sigma = 0.1, lower = 0
y0_resources = [1,1]
lower growth rate
...
print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Starting CRM Full model...", flush=True)
...
idata, idata_prior = inference.run_inference()
...
print(f"[{datetime.now().strftime('%H:%M:%S')}] CRM Full complete", flush=True)
...
# Extract the last value from each list
log_ml_list = []
...
print(f"\nCRM Full - Log ML per chain: {log_ml}")

if len(log_ml) > 0:
    print(f"CRM Full - Log ML: {np.mean(log_ml):.4f} ± {np.std(log_ml):.4f}")
...
# idata = az.from_netcdf('/home/cclare/projects/CRM/MIMIC/examples/CRM/Clare_EcN_Sent_EAB12glu_test_norm6/model_posterior.nc')
# idata_prior = az.from_netcdf('/home/cclare/projects/CRM/MIMIC/examples/CRM/Clare_EcN_Sent_EAB12glu_test_norm6/model_prior.nc')
...
for i in range(n_samples):
    ...
    sample_species, sample_resources = sample_predictor.simulate(times, init_conditions)
...
# Resources (no noise - not observed)
...
param_vars = ["tau_hat", "w_hat", "c_hat", "m_hat", "r_hat", "K_hat"]
#param_vars = ["w_hat", "c_hat", "m_hat"]
...
posterior_array = np.column_stack([samples_dict[name] for name in param_names])

inference = inferCRMbayes()
inference.plot_corner_topright(param_names, posterior_array, prior_means, prior_sigmas)

plt.savefig(os.path.join(output_folder, 'corner_plot_parameters_top.jpg'), dpi=300, bbox_inches='tight')
plt.close()
...
param_list = ['tau_hat', 'w_hat', 'c_hat', 'm_hat', 'r_hat', 'K_hat', 'sigma']
...
plot_params = ['tau_hat', 'w_hat', 'c_hat', 'm_hat', 'r_hat', 'K_hat']
