...
# Ensemble size variable: you can modify this value
ensemble_size = 10
...
def glv_pde_with_unknown_perturbation(t, y, trainable_params, num_species, s_M, s_eps, lambda_pde):
    ...
    The residuals are multiplied by lambda_pde to weight their contribution.
    """
...
def infer_parameters_from_file(sim_file):
    """
    Trains a single PINN on the simulation data from sim_file.
    Returns a dict with true and inferred parameters.
    """
    ...
    # Initial guess for parameters: [mu_guess, M_guess_scaled, eps_guess_scaled]
    num_params_mu = num_species
    num_params_M = num_species * num_species
    mu_guess = np.random.uniform(0.8, 1.6, size=num_species)
    M_guess = np.random.uniform(-0.03, 0.06, size=(num_params_M,))
    for i in range(num_species):
        # Ensure the diagonal elements are negative
    ...
    # Build the neural network with L2 regularization added to each layer.
    ...
    plt.legend()
    plt.tight_layout()
    plot_filename = "inference_results/" + \
...
def ensemble_infer_parameters(sim_file, ensemble_size=ensemble_size):
    """
    Trains an ensemble of PINN models for a given simulation file.
    Returns the aggregated (mean and std) inferred parameters and the list of individual results.
    """
    ensemble_results = []
    for i in range(ensemble_size):
        print(
            f"Ensemble run {i+1}/{ensemble_size} for {os.path.basename(sim_file)}")
        result = infer_parameters_from_file(sim_file)
        ensemble_results.append(result)

    # Convert list to arrays for statistical aggregation.
    inferred_mu_all = np.array([res["inferred_mu"]
                               for res in ensemble_results])
    inferred_epsilon_all = np.array(
        [res["inferred_epsilon"] for res in ensemble_results])
    inferred_M_all = np.array([res["inferred_M"] for res in ensemble_results])

    aggregated = {
        "simulation_file": os.path.basename(sim_file),
        "true_mu": ensemble_results[0]["true_mu"],
        "true_M": ensemble_results[0]["true_M"],
        "true_epsilon": ensemble_results[0]["true_epsilon"],
        "mu_mean": inferred_mu_all.mean(axis=0).tolist(),
        "mu_std": inferred_mu_all.std(axis=0).tolist(),
        "epsilon_mean": inferred_epsilon_all.mean(axis=0).tolist(),
        "epsilon_std": inferred_epsilon_all.std(axis=0).tolist(),
        "M_mean": inferred_M_all.mean(axis=0).tolist(),
        "M_std": inferred_M_all.std(axis=0).tolist(),
...
def main():
    ...
    for sim_file in sim_files:
        print(f"Processing ensemble for {sim_file} ...")
        ensemble_result = ensemble_infer_parameters(sim_file, ensemble_size)
        all_ensemble_results.append(ensemble_result)

        # Save the aggregated ensemble result for each simulation file.
        out_file = os.path.join(out_dir, os.path.basename(
            sim_file).replace(".json", "_ensemble_inferred.json"))
        with open(out_file, "w") as f:
            json.dump(ensemble_result, f, indent=4)
        print(f"Saved ensemble inferred parameters to {out_file}")

    # Save a combined file for all simulations.
    combined_file = os.path.join(
        out_dir, "all_ensemble_inferred_parameters.json")
    with open(combined_file, "w") as f:
        json.dump(all_ensemble_results, f, indent=4)
    print(f"Saved all ensemble inferred parameters to {combined_file}")
