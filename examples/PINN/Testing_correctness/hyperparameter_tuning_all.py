...
def train_and_evaluate(sim_file, lambda_pde, reg_coeff, lr, tuning_iterations=5000):
    ...
    model = build_model(t_data, noisy_solution, num_species,
                        trainable_params, lambda_pde, reg_coeff, lr)
...
def hyperparameter_tuning_all(sim_dir="simulations", tuning_iterations=5000):
    ...
    # Grid search: try each combination across all simulations
    for lambda_pde_val, reg_coeff, lr in product(lambda_pde_values, reg_coeff_values, learning_rates):
        mses = []
        print(
            f"Evaluating hyperparameters: lambda_pde={lambda_pde_val}, reg_coeff={reg_coeff}, lr={lr}")
        for sim_file in sim_files:
            mse = train_and_evaluate(
                sim_file, lambda_pde_val, reg_coeff, lr, tuning_iterations=tuning_iterations)
            mses.append(mse)
            print(f"  Simulation {os.path.basename(sim_file)}: MSE={mse:.4e}")
    ...
    print(
        f"Best hyperparameters across all simulations: {best_config} with average MSE: {best_avg_mse:.4e}")
    return best_config


def main():
    best_config = hyperparameter_tuning_all(
        sim_dir="simulations", tuning_iterations=5000)
    out_dir = "inference_results"
    os.makedirs(out_dir, exist_ok=True)
    # Save the best hyperparameters to a file so they can be used by the final training script
    best_config_file = os.path.join(out_dir, "best_hyperparameters.json")
    with open(best_config_file, "w") as f:
        json.dump(best_config, f, indent=4)
    print(f"Best hyperparameters saved to {best_config_file}")
